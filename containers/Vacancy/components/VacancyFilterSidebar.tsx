...
interface VacancyFilterSidebarProps {
    open: boolean
    onClose: () => void
    onApplyFilter: (filters: { id: number; attributeValueIds: number[] }[]) => void
}

export default function VacancyFilterSidebar({ open, onClose, onApplyFilter }: VacancyFilterSidebarProps) {
    const t = useTranslations("cvFilter")
    const tc = useTranslations("vacancyCreate")
