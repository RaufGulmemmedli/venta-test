...
export default function ProcessPage({ cvData }: ProcessPageProps) {
  const [searchTerm, setSearchTerm] = useState("")

  // Static process data based on the image - will be dynamic later
  const processData = [
    {
      id: 1,
      proposer: "GÜNAY NURİYEVA KAMAL QIZI -- Nuriyeva.Gunay@venta.az",
      operation: "",
      changes: "Məlumat tapılmadı",
      status: "Cv Yaradıldı",
      daysInProcess: "Gün təyin edilməyib",
      operationDuration: "19:19 dəq.",
    ...
    },
    {
      ...
      changes: "CV məlumatları yeniləndi",
      status: "İnkişaf mərhələsində",
      daysInProcess: "3 gün",
      operationDuration: "5:30 dəq.",
    ...
    },
    {
      ...
      operation: "Müsahibə planlaşdırıldı",
      changes: "Müsahibə tarixi müəyyən edildi",
      status: "Müsahibə gözləyir",
      daysInProcess: "1 gün",
      operationDuration: "12:45 dəq.",
  ...
  const getStatusColor = (status: string) => {
    if (status.includes("Yaradıldı")) return "bg-green-600"
    if (status.includes("İnkişaf")) return "bg-blue-600"
    if (status.includes("gözləyir")) return "bg-red-600"
    if (status.includes("təyin edilməyib")) return "bg-gray-400"
    return "bg-gray-400"
  }

  const getChangesColor = (changes: string) => {
    if (changes.includes("tapılmadı")) return "text-red-600"
